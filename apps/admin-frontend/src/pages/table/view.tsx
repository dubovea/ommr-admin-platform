...
export function TableListPage() {
  ...
  const columns = useMemo<ColumnDef<AdminTableMeta>[]>(
    () => [
      ...
      { id: "label", accessorKey: "label", header: "Название" },
      { id: "fieldsCount", accessorKey: "fieldsCount", header: "Поля" },
      { id: "relationsCount", accessorKey: "relationsCount", header: "Связи" },
      {
        id: "status",
        accessorKey: "status",
        header: "Статус",
  ...
  const table = useTable<AdminTableMeta>({
    ...
    refineCoreProps: { resource: "tables", pagination: { mode: "off" } },
  });
  const reactTable = getTanStackTable<AdminTableMeta>(table as any);
  const rows = reactTable.getRowModel().rows;
  useEffect(() => {
    if (rows.length === 0 || Object.keys(rowSelection).length > 0) return;
    const ordersRow = rows.find((row) => row.original.name === "orders");
    const rowToSelect = ordersRow ?? rows[0];
    setRowSelection({ [rowToSelect.id]: true });
  }, [rows.length, rowSelection]);
  const selectedTable =
    reactTable.getSelectedRowModel().rows[0]?.original ??
    rows[0]?.original ??
    null;
  return (
    <div className="grid grid-cols-[minmax(0,1fr)_420px] gap-6">
      <ListView>
        ...
        <DataTable
          ...
          getRowId={(row) => row.id}
          onRowClick={(row) => setRowSelection({ [row.id]: true })}
        />
      </ListView>
